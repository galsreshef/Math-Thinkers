...
public class ScoreTimeAttack extends Fragment {

    Context context;

    public ScoreTimeAttack() {
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        final RecyclerView rvTimeAttack = view.findViewById(R.id.rvGameScore);
        TextView tvGameSelected = view.findViewById(R.id.tvOwnScore);
        tvGameSelected.setText(getResources().getText(R.string.time_attack_game));
        final List<Score> timeAttackScoreList = new ArrayList<>();

        DatabaseReference refTimeAttack = FirebaseDatabase.getInstance().getReference().child("Score").child("Games").child("TimeAttack");


        Query queryTimeAttack = refTimeAttack.orderByChild("Score").limitToLast(5);
        queryTimeAttack.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        Score scoreObject = new Score();
                        scoreObject.setType("TimeAttack");
                        ...
                        timeAttackScoreList.add(scoreObject);
                    }
                    Collections.reverse(timeAttackScoreList);
                    rvTimeAttack.setHasFixedSize(true);
                    rvTimeAttack.setLayoutManager(new LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false));
                    final ScoresAdapter feedAdapter = new ScoresAdapter(context, timeAttackScoreList);
//                    rvTimeAttack.setPadding(0, 220, 0, 0);
                    rvTimeAttack.setAdapter(feedAdapter);
                }
            }
